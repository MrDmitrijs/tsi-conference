...
class ScheduleView extends Component {
    ...
    render() {
        return (
            <View>
                {/*{this.state.days && <SegmentedControlIOS*/}
                ...
                <SectionList
                    ...
                    return <ListItem containerStyle={{borderLeftWidth: 6}} key={index}
                                  ...
                                  onPress={() => this.setState({session: session, modal: true, section: section})}/>}
                    }
                        ...
                        const startDate = new Date(section.start),
                            ...
                            placeSubtitle = isMultiPlaces ? <SegmentedControlIOS
                                ...
                                onChange={(event) => {
                                    ...
                                    sessionsAndPlaces.set(section.start, {places: sessionsAndPlaces.get(section.start).places, selectedIndex: event.nativeEvent.selectedSegmentIndex});
                                    this.setState({
                    ...
                    <Modal isVisible={this.state.modal}
                        ...
                        <Card title={(this.state.session.name || '').toUpperCase()}>
                            <View>
                                ...
                                <Text
                                    style={{textAlign: 'justify', marginTop: 10}}>{this.state.session.description}</Text>
                                {this.state.session.speaker ?
